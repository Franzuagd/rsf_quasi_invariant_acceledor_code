...
def plot_invariant_section(Ix, Iy, idx_to_vec, eps, a_box, plane="both", levels=25, n=350, rmin=0.06, rmax=0.95, delta0=0.0, folder="graficos_invariantes"):
    plane = plane.lower()

    if plane == "both":
        outx = plot_invariant_section(Ix, Iy, idx_to_vec, eps, a_box, "x", levels, n, rmin, rmax, delta0, folder)
        outy = plot_invariant_section(Ix, Iy, idx_to_vec, eps, a_box, "y", levels, n, rmin, rmax, delta0, folder)
        return {"x": outx, "y": outy}

    if plane == "x":
        I, qmax, pmax, qlab, plab, title = Ix, float(a_box[1]), float(a_box[3]), "x", "px", "Ix on y=py=0"
    elif plane == "y":
        I, qmax, pmax, qlab, plab, title = Iy, float(a_box[2]), float(a_box[4]), "y", "py", "Iy on x=px=0"
    else:
    ...
    cs = plt.contour(Q, P, Z, levels=lev)
...
def testing(v, tol=1e-14):
    ...
    Sx = np.zeros(quad_size, dtype=float)
    Sx[idx_x2] = gx / epsilon[idx_x2]
    Sx[idx_xpx] = 2.0 * ax / epsilon[idx_xpx]
    Sx[idx_px2] = bx / epsilon[idx_px2]

    Sy = np.zeros(quad_size, dtype=float)
    Sy[idx_y2] = gy / epsilon[idx_y2]
    Sy[idx_ypy] = 2.0 * ay / epsilon[idx_ypy]
    Sy[idx_py2] = by / epsilon[idx_py2]
    ...
    plot_invariant_section(Ix, Iy, idx_to_vec, epsilon, a_box, plane="both", levels=25)
...
def main():
	...
	d = 1
	n = 2  #Fixed for now!
	a_box = np.array([0.1e-2, 4e-3, 0.1e-3, 0.8e-3, 0.1e-3], dtype=float)
	H_pol = (
	...
	chry=0
	schr1='d2'
	schr2='d3'; varelem  = ['d1','d4','d3d','d4d','o2','o3','o4','o3d','o4d']
	...
	v0=[3.148346595634115e+03*3,-4.621730496569649e+02*3,1.366797632793281e+02*3,-1.713849194855147e+02*3,-2.526041097299107e+12*1e-8,4.591682297815766e+07*1e-8,-3.136093044673090e+11*1e-8,-7.400633479124564e+10*1e-8,6.886521769314347e+11*1e-8]
...
#		plt.plot(st,dispt,label='$100*dD/ds$',linewidth=3.0,color='yellow')
	...
	solution, fobj, ga_instance = optimize(v0,num_generations=100,sol_per_pop=40,num_parents_mating=4,save_csv=True,csv_name="resultados.csv",fobj_csv_name="f_obj_dinamica.csv",opa_txt_name="bloque_opa_final.txt",reset_saved_files=True)
	testing(solution)
	#print("Fobj:")
	#print(fobj)
