...
def invariant(tnn, tnq, Sx, Sy, tol=1e-6):
...
def objfunc_given_linear_data(v, bx, ax, gx, by, ay, gy, csft, dispt, print_table=False, save_csv=False):
    try:
    ...
    fobj = (nhx + err_x * 10**2) / max(Normx, 1e-300) + (nhy + err_y * 10**2) / max(Normy, 1e-300)
    ...
    if save_csv:
        ...
        solution = np.append(solution, rel_err_x)
        solution = np.append(solution, rel_err_y)
        solution = np.append(solution, Normx)
        solution = np.append(solution, Normy)
...
def save_solution_csv(csv_name, solution, fobj, solution_fitness=None, solution_idx=None, stage="generation"):
    ...
    print(f"Saved CSV in: {os.path.abspath(csv_name)}", flush=True)
...
def optimize(v0, num_generations=5, sol_per_pop=20, num_parents_mating=3,
    ...
    def on_generation_callback(ga_instance):
        ...
        fobj = objfunc(solution, print_table=True)
        ...
        if save_csv:
            save_solution_csv(
                ...
                stage="generation_" + str(generation)
            )
...
def main():
	...
	varelem  = ['d1','d4','d3d','d4d','o2','o3','o4','o3d','o4d']
	v0=[3.148346595634115e+03*3,
	...
	bxP = bx; axP = ax; gxP = gx; byP = by; ayP = ay; gyP = gy
