...
def nulm2e(elem, H_vec, M_basis, tol=1e-12, check_uper_right=False):
    """
    ...
    if elem.length == 0:
        ...
        A = (L * M).tocsc()
        tmatrix = sps.linalg.expm(A).toarray()
...
def element_generator(elem, H_vec, M_basis):
    """
    Construye A_e = L_e M_e como matriz sparse.
    No calcula exp(A_e).
    """
...
def build_transport_generators(cell, H_vec, M_basis):
    """
    Construye la lista [A_1, A_2, ..., A_N],
    donde A_j = L_j M_j.

    Esto se hace una vez por evaluación de objfunc.
    """

    generators = []

    for i in cell:
        A = element_generator(cell[i], H_vec, M_basis)
        generators.append(A)

    return generators

def apply_transport(v, generators):
    """
    Aplica Phi v sin construir Phi.

    Phi = exp(A_N) ... exp(A_2) exp(A_1)
    """
    ...
    for A in generators:
        if A is not None:
            out = sps.linalg.expm_multiply(A, out)

    return out

def apply_transport_transpose(v, generators):
    """
    Aplica Phi^T v sin construir Phi.

    Si Phi = exp(A_N) ... exp(A_1),
    entonces Phi^T = exp(A_1)^T ... exp(A_N)^T.
    """

    out = np.array(v, dtype=float).copy()

    for A in reversed(generators):
        if A is not None:
            out = sps.linalg.expm_multiply(A.T, out)

    return out

def embed_quadratic_vector(S):
    """
    Convierte S en vector completo [S, 0].
    """

    full = np.zeros(quad_size + nonquad_size, dtype=float)
    full[:quad_size] = S

    return full


def embed_nonquadratic_vector(h):
    """
    Convierte h en vector completo [0, h].
    """
...
def project_nonquadratic_vector(v):
    """
    Extrae la parte no cuadrática P_n v.
    """

    return v[quad_size:]

def matrix_free_U(S, generators):
    """
    Calcula U = P_n Phi [S, 0],
    sin construir Phi.
    """

    full_S = embed_quadratic_vector(S)
    transported = apply_transport(full_S, generators)
    U = project_nonquadratic_vector(transported)

    return U

def matrix_free_D(h, generators):
    """
    Calcula D h = h - P_n Phi [0, h],
    sin construir D ni Phi.
    """

    full_h = embed_nonquadratic_vector(h)
    transported = apply_transport(full_h, generators)
    lower = project_nonquadratic_vector(transported)

    return h - lower

def matrix_free_DT(z, generators):
    """
    Calcula D^T z sin construir D.

    D = I - P_n Phi E_n
    D^T = I - E_n^T Phi^T P_n^T
    """

    full_z = embed_nonquadratic_vector(z)
    transported_back = apply_transport_transpose(full_z, generators)
    lower_back = project_nonquadratic_vector(transported_back)

    return z - lower_back

def invariant_matrix_free(generators, Sx, Sy, Gnn, atol=1e-10, btol=1e-10, maxiter=None):

    print("Computing Ux...", flush=True)
    Ux = matrix_free_U(Sx, generators)

    print("Computing Uy...", flush=True)
    Uy = matrix_free_U(Sy, generators)

    Lg = Lgnn

    def matvec(h):
        Dh = matrix_free_D(h, generators)
        return Lg.T @ Dh

    def rmatvec(y):
        z = Lg @ y
        return matrix_free_DT(z, generators)

    Aop = sps.linalg.LinearOperator(
        shape=(nonquad_size, nonquad_size),
        matvec=matvec,
        rmatvec=rmatvec,
        dtype=float
    )
    ...
    print("Solving x with lsmr...", flush=True)
    solx = sps.linalg.lsmr(
        Aop, bx,
        atol=atol,
        btol=btol,
        maxiter=maxiter,
    ...
    print("Solving y with lsmr...", flush=True)
    soly = sps.linalg.lsmr(
        Aop, by,
        atol=atol,
        btol=btol,
        maxiter=maxiter,
    ...
    residual_x = Ux - matrix_free_D(hx, generators)
    residual_y = Uy - matrix_free_D(hy, generators)
    ...
    print("LSMR x info:", solx[1], solx[2], solx[3], flush=True)
    print("LSMR y info:", soly[1], soly[2], soly[3], flush=True)
...
def objfunc(v, print_table=False, save_csv=False):
    ...
    generators = build_transport_generators(cell, H_vec, M_basis)

    nhx, nhy, err_x, err_y = invariant_matrix_free(
    generators, Sx, Sy, Gnn,
    atol=1e-8,
    btol=1e-8,
    maxiter=20
)

    fobj = (nhx+ err_x*10**2)/(Normx) + (nhy + err_y*10**2)/(Normy)  #Error << 1000
...
def optimize(v0, num_generations=5, sol_per_pop=20, num_parents_mating=3,
    ...
    fobj0 = objfunc(v0, print_table=True)
    ...
    def on_generation_callback(ga_instance):
        ...
        print('=' * term_columns, flush=True)
        print(np.array(solution, dtype=float), flush=True)
        print('=' * term_columns, flush=True)
    ...
    if save_csv:
        ...
        pd.DataFrame(row.reshape(1, -1)).to_csv(
            csv_name,
            mode="a",
            index=False,
            header=False
...
def main():
	...
	# quad_non_quad(bx, ax, gx, by, ay, gy)
	generators = build_transport_generators(cell, H_vec, M_basis)
	vtest= np.zeros(len(idx_to_vec))
	vtest[0]=1.0; out = apply_transport(vtest, generators)
	print(out.shape)
	print(np.linalg.norm(out))
	#solution, fobj, ga_instance = optimize(v0, 50, 20)
